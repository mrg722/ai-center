      ...
      <div className="flex flex-col gap-3 p-3 sm:p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-1 text-[10px] font-mono">
            <Badge>{data ? data.total + ' modelos' : 'cargando…'}</Badge>
            <Badge>{data ? data.providers.length + ' proveedores' : '—'}</Badge>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-line pt-2">
          ...
          <div className="flex gap-2">
            {data?.agent && <button onClick={() => onSelectAgent?.(data.agent!.id)} className="rounded bg-[#76b900] px-3 py-1.5 text-[11px] font-semibold text-black">Hablar con NVIDIA</button>}
