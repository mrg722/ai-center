...
export const POST=userRoute(async({req,db,user})=>{
 const project=await requireProject(db);const body=await readJson(req,async()=>{const x=await req.json() as {model?:unknown};if(typeof x.model!=='string'||x.model.length<3||x.model.length>200)throw new HttpError(400,'model inválido');return x});
 ...
 const a=await getAgentBySlug(db,project.id,'vercel');if(!a)throw new HttpError(409,'agente Vercel no inicializado');await db.query('update agents set model=$2,enabled=true where id=$1',[a.id,m.id]);await emit(db,{project_id:project.id,type:'agent.updated',actor:{kind:'user',id:user.id,name:user.display_name},agent_id:a.id,payload:{model:m.id,provider:'vercel-ai-gateway'}});const av=await agentViews(db,project),updated=av.find(x=>x.id===a.id);if(!updated)throw new HttpError(500,'agente Vercel no disponible');return {ok:true,agent:updated};
});