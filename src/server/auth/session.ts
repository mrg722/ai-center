...
interface SessionPayload {
  uid: string;
  v: number; // users.session_version — bump to revoke all sessions
  exp: number;
}
...
export async function createSession(user: Pick<UserRow, 'id' | 'session_version'>): Promise<void> {
  const token = sign({ uid: user.id, v: user.session_version, exp: Math.floor(Date.now() / 1000) + MAX_AGE_S } satisfies SessionPayload, env.sessionSecret);
